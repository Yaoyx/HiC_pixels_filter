...
def test_cis_total_filter(thres, clr_test=None):
    if clr_test == None:
        ...
        bins = pd.DataFrame(
            [["chr1", 0, 5], ["chr1", 5, 8], ["chrX", 0, 5], ["chrX", 5, 10]],
        ...
        # = 0.5 cis-total ratio
        pixels = pd.DataFrame(
            [[0, 0, 6], [1, 2, 3], [1, 3, 35], [2, 2, 3], [2, 3, 30], [3, 3, 5]],
    ...
    output_path = (
        f"/home1/yxiao977/sc1/train_akita/test_data/test_data_util_{thres}filtered.cool"
    )
    bin_mask = data_util.generate_bin_mask(
        clr_test, [data_util.cis_total_ratio_filter(0.5)]
    )
    data_util.create_filtered_cooler(
        output_path, clr_test, bin_mask, chunksize=10_000_000, nproc=16
    )
